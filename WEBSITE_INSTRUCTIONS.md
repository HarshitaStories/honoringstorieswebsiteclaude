...
- **About** (`about.html`, built): no standalone photo/name hero banner (one was tried and then
  ...
  - **The bio portrait is the prepared `aboutfinal` photograph across phone, tablet and laptop.**
    ...
    `.full-photo` uses a 400px maximum width and its natural landscape ratio, with no cropping
    or stretching, so the plant, chair and table remain in the composition. Two intersecting
    linear masks soften the outer 12% on all four edges into the page cream on every device.
    The taller 4:5 cover crop was tried and rejected because it removed parts of the scene.
...
- **Work With Me, Supervision** (`supervision.html`, built): same pattern, using
  `assets/supervision-hero.jpg`. Includes a **three-track** section ("Wherever you are in your
...
13. **A change is not finished until five things are done**, and the last four are the ones
    ...
    accident six months later. From 2026-08-31 onward, every website change must also be recorded in
    `codex_WEBSITE_INSTRUCTIONS.md`, including tiny visible copy changes. That file is the
    Codex-maintained standalone mirror of the complete frontend, backend, reasoning and version
    history. If a practical guide changes, update its matching file in `codex_spoonfed/` too. If
    `AGENTS.md` changes, update `codex_AGENTS.md` in the same commit.
...
36px page gutters. Tablet and desktop layouts remain unchanged.
...
| Decision / change | Reason |
| --- | --- |
| Replaced the laptop landing-page preview with Harshita's portrait | All nine public pages now use `assets/portrait-preview.png`, a 1200 by 630 image containing only the seated black-top portrait on the site cream. Preview titles and descriptions remain separate metadata. The previous laptop-page image is retained but unused. Sharing apps still control whether they show the expanded card or a compact thumbnail. |
...
| Change | Why |
| --- | --- |
| Removed the final full stop after "heavy" in the homepage headline | Harshita requested that the displayed hero headline end without the dot. |
| Created `codex_WEBSITE_INSTRUCTIONS.md`, `codex_AGENTS.md` and `codex_spoonfed/` | Harshita requested a clearly identified Codex-maintained copy preserving the complete history, working rules and practical setup. Every future website change must update the standalone Codex record, and matching practical copies when relevant. |
| Deployment handoff fixed as push, then Hostinger | Codex commits and pushes every completed change. Harshita clicks Deploy at the existing Hostinger Git deployment page. |
